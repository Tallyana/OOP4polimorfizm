...
public class Main {
    public static void main(String[] args) {
        ...
        truck1.setDiagnosticsPassed(false);
        try {
            checkTransport(bus4, truck6, truck3);
    ...
    public static void checkTransport(Transport...transports){
        ...
        for(Transport transport: transports){
            if(!transport.passDiagnostics()) {
                try {
                    throw new RuntimeException(transport.setBrand() + " " + transport.setModel() + "не прошел диагностику!");
